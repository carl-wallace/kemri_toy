#!/bin/bash
# Generate signed PKCS #10 CSRs for all supported signature algorithms.

set -e

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
OUTPUT_DIR="${1:-$SCRIPT_DIR/artifacts_csrs}"
...
# All signature algorithms that kemri_toy can generate CSRs for.
# Mirrors gen_all_certs.sh; excludes algorithms that hit todo!() (ed25519, ed448 variants).
ALGORITHMS=(
    # Pure ML-DSA
    ml-dsa44
    ml-dsa65
...
for alg in "${ALGORITHMS[@]}"; do
    echo -n "Generating CSR for $alg... "
    if "$KEMRI_TOY" --generate-csr --sig "$alg" --output-folder "$OUTPUT_DIR" 2>/dev/null; then
        echo "ok"
        SUCCESS=$((SUCCESS + 1))
    else
        echo "FAILED"
...
echo ""
echo "Generated $SUCCESS/$TOTAL CSRs successfully."
if [ $FAIL -gt 0 ]; then
